...
const validateWord = (word) => {
  const ptrn = /\b[a-zA-Z]{5,10}\b/;
  return ptrn.test(word);
};
...
const hasNumber = (string) => {
  const ptrn = /[a-zA-Z]+[0-9]+/;
  return ptrn.test(string);
};
...
const validatePhoneNumber = (phoneNumber) => {
  const ptrn = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
  return ptrn.test(phoneNumber);
};
